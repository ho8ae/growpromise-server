 ...
 */
export const createPromise = asyncHandler(async (req: Request, res: Response) => {
  if (!req.user) {
  ...
  const { title, description, repeatType, startDate, endDate, childIds } = req.body;
 ...
 */
export const getParentPromises = asyncHandler(async (req: Request, res: Response) => {
  if (!req.user) {
    return res.status(401).json({
      success: false,
      message: '인증이 필요합니다.'
 ...
 */
export const getChildPromises = asyncHandler(async (req: Request, res: Response) => {
  if (!req.user) {
    return res.status(401).json({
      success: false,
      message: '인증이 필요합니다.'
  ...
  const promises = await promiseService.getChildPromises(req.user.id, status as any);
 ...
 */
export const getPromiseById = asyncHandler(async (req: Request, res: Response) => {
  if (!req.user) {
    return res.status(401).json({
      success: false,
      message: '인증이 필요합니다.'
 ...
 */
export const updatePromise = asyncHandler(async (req: Request, res: Response) => {
  if (!req.user) {
    return res.status(401).json({
      success: false,
      message: '인증이 필요합니다.'
  ...
  const promise = await promiseService.updatePromise(id, req.user.id, updateData);
 ...
 */
export const deletePromise = asyncHandler(async (req: Request, res: Response) => {
  if (!req.user) {
    return res.status(401).json({
      success: false,
      message: '인증이 필요합니다.'
...
export const respondToVerification = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.status(200).json({
    success: true,
    message: approved ? '약속 인증이 승인되었습니다.' : '약속 인증이 거절되었습니다.',
    data: result
  });
});
 ...
 */
export const getChildPromiseStats = asyncHandler(async (req: Request, res: Response) => {
  if (!req.user) {
    return res.status(401).json({
      success: false,
      message: '인증이 필요합니다.'
